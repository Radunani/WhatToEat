...
protocol MealDBServiceProtocol {
    ...
    func listMeals(byFirstLetter letter: Character) async throws -> [Meal]
    func lookupMeal(byID id: String) async throws -> Meal?
    func randomMeal() async throws -> Meal?
    func listMealCategories() async throws -> [MealCategory]
...
final class MealDBAPIService: MealDBServiceProtocol {
    ...
    func listItems(for type: MealListType) async throws -> MealListItems {
        ...
        case .ingredient:
            ...
            return .ingredients(response.meals ?? [])
        }
    }
